...
def pearsonMatrix(ratingMatrix):
    noitems = np.shape(ratingMatrix)[1]
    meanMat = np.zeros((noitems))
    for i in xrange(noitems):
        meanMat[i] = np.mean(ratingMatrix[:, i])
    sim = np.zeros((noitems, noitems))
    for i in xrange(noitems):
        for j in xrange(i, noitems):
            sim[i][j] = cosine(ratingMatrix[:, i] - meanMat[i], ratingMatrix[:, j] - meanMat[j])
            sim[j][i] = sim[i][j]
            # print i
    return sim
